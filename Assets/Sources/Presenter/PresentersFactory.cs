...
public class PresentersFactory : MonoBehaviour
{
    ...
    {
        CreatePresenter(_nloTemplate, nlo, nlo.Team);
    }
    ...
    private Presenter CreatePresenter(Presenter template, Transformable model, int TeamNlo)
    {
        Presenter presenter = Instantiate(template);
        presenter.Init(model, _camera);
        if (TeamNlo==1)
        {
            presenter.GetComponent<SpriteRenderer>().color = Color.red;
        }
        else
        {
            presenter.GetComponent<SpriteRenderer>().color = Color.blue;
        }
        return presenter;
    }
}
